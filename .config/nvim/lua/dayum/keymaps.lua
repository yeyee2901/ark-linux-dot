...
-- SEARCH
map('n', 'S', ':%s/', opts)
map('n', '<S-h>', '<cmd>set hlsearch!<CR>', silent)
...
-- useful for moving line up and down like Alt+Up & Alt+Down
-- in VSCode
map('v', '<M-k>', ':m-2<CR>gv=gv',    silent)
map('v', '<M-j>', ":m '>+1<CR>gv=gv", silent)

-- append to selected line
map('v', 'aa', '$<S-a>', opts)
...
map('n', '<C-s>', '<cmd>Telescope ultisnips<CR>', opts)
map('i', '<C-s>', '<cmd>Telescope ultisnips<CR>', opts)
...
map('n', '<Leader>gh', '<cmd>diffget //2<CR>', opts)
map('n', '<Leader>gl', '<cmd>diffget //3<CR>', opts)
